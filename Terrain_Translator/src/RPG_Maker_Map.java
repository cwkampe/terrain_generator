...
public class RPG_Maker_Map {
	...
	int get_auto_tile_value (int base, int index, char[] terrain) {
		...
		} else temp_offset += 32; } else temp_offset += 32;
		...
		} else temp_offset += 16; } else temp_offset += 16;
		...
		} else temp_offset += 64; } else temp_offset += 64;
		//check bottom center
		if (check_bounds(temp[6])) { if (terrain[temp[6]] == terrain[index]){ /*do nothing*/ } else temp_offset += 128; } else temp_offset += 128;
